...
const CityClock = ({ city, zone, onDelete }: { city: string; zone: string; onDelete?: () => void }) => {
   ...
   return (
      <motion.div
         ...
         border border-white/20 w-[calc(100%/3.2)] h-[calc(250px)]
         rounded-2xl shadow-xl flex flex-col items-center 
         justify-center aspect-square transition-all preserve-3d"
      >
         {onDelete && (
            <button
               onClick={onDelete}
...
const WorldClock = () => {
   ...
   return (
      <div className="w-[90%] max-w-6xl mx-auto h-[90%] overflow-y-auto no-scrollbar">
         <div className="flex justify-center items-center mb-4 px-4 w-full">
            ...
            tracking-widest uppercase top-11 z-10 flex justify-center 
            items-center left-1/2 transform -translate-x-1/2 space-x-4 
            items-center mb-8 px-4 w-full">
         ...
         {showForm && (
            <motion.div
               ...
               mb-8 overflow-hidden w-[90%] mx-auto "
            >
         ...
         <div className="flex flex-wrap gap-2 opacity-90 h-full w-full 
